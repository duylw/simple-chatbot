"""Database Seeder Module (CSV & Chroma Vector Import)."""
import csv
import os
import pickle
import uuid
...
from src.infrastructure.database.models import VideoModel, ChunkModel
...
async def ensure_user_schema(session: AsyncSession):
    """Ensure that the users table contains hashed_password column."""
...
async def seed_db_if_empty(session: AsyncSession):
    """Seed relational database from CSV files if videos table is empty."""
    result = await session.execute(select(VideoModel).limit(1))
    if result.scalars().first() is not None:
        logger.info("Database already has data, skipping relational seeding.")
        return

    logger.info("Seeding database with CSV data...")
    videos = {}
    video_csv_path = os.path.join(DATA_DIR, "videos.csv")
    if os.path.exists(video_csv_path):
        with open(video_csv_path, mode="r", encoding="utf-8") as f:
    ...
    chunks = {}
    chunk_csv_path = os.path.join(DATA_DIR, "chunks.csv")
    if os.path.exists(chunk_csv_path):
        with open(chunk_csv_path, mode="r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
    ...
    map_csv_path = os.path.join(DATA_DIR, "video_chunks.csv")
...
async def seed_vector_db_if_empty():
    """Seed ChromaDB from pickle file if empty."""
    try:
        ...
        chroma_client._collection.add(
            ...
            metadatas=loaded_data.get("metadatas", None),
