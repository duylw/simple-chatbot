"""Academic Prompt Templates for Agent Guardrail, HyDE, Generation and Grading."""
...
4. **Adaptive Rewriting & Multi-Turn Reflection:**
   - **Original Intent (Bắt buộc giữ vững):** Always ensure the core technical question from the student's original query is strictly preserved, avoiding semantic drift across iterations.
   - **Iteration 2+ (Khi có đoạn thử trước và Gợi ý):** Analyze what aspects the `Previous Hypothetical Document` might have missed or why it failed to find slides. Strictly integrate the `Reflection Suggestion` and targeted terminology to pivot the technical description towards the correct lecture materials.
   - **Iteration 1 (Lần đầu):** Focus purely on converting the original question into a formal academic lecture snippet.
5. **CRITICAL - No Hallucination:**
...
---
### Input Data
- **Student Original Query (Câu hỏi gốc):** {original_query}
- **Previous Hypothetical Document (Đoạn mô tả đã thử lần trước):** {previous_rewritten_query}
- **Reflection Suggestion (Gợi ý điều chỉnh từ lần chấm điểm trước):** {suggestion}
